...
public class changeKey_cl extends JP.go.ipa.oz.system.OzCell implements JP.go.ipa.oz.user.secu.changeKey_if {
...
// 9
    if (isequal$val$2) {
// 10
      JP.go.ipa.oz.lib.standard._String_if string$1 = (new JP.go.ipa.oz.lib.standard._String_cl("usage:changeKey userName keySeed"));
...
// 14
        JP.go.ipa.oz.lib.standard._OzKey_if changeKey$val$4;
        if (changeThreadRedIfNecessary ((JP.go.ipa.oz.system.OzSecure) as)) {
          try {
            changeKey$val$4 = as.changeKey (userName, keySeed);
          } finally {
            changeRunningToGreen ();
          }
        } else {
  ...
  public changeKey_cl () { super ("JP.go.ipa.oz.user.secu.changeKey_pcl"); }
  
  public changeKey_cl (String proxy_id) { super (proxy_id); }
